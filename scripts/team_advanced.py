...
df['hAST%'] = ts['AST'] / (ts['FGA'] + (0.44 * ts['FTA']) + ts['AST'] + ts['TO'])
df['hTO%'] = ts['TO'] / (ts['FGA'] + (0.44 * ts['FTA']) + ts['AST'] + ts['TO'])
...
df.to_csv('../current_stats/team_advanced.csv')
df.to_csv('../team_stats/team_advanced_{0}.csv'.format(game_id))